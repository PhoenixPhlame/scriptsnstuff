...
if (sys.getVal(sys.name(src) + "emotes") == "true") {
	if (Config.emotes.indexOf(command) !== -1) {
		...
		sessUser.lastEmote = (+sys.time()) + emoteTimeout;
...
//Emote Timeout
if (command == "timemote"){
emoteTimeout = commandData;
sm(src, "You have set the emote timeout to: "+commandData+". The current emote timeout is now: "+emoteTimeout+"", channel);
sa("+Pikachu: "+sys.name(src)+" has set the emote timeout to: "+commandData+". The current emote timeout is now: "+emoteTimeout+"", staffchannel);
return;
}
if (command == "smute") {
